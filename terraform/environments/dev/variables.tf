...
variable "github_app_repo" {
  description = "Name of the application repository whose workflow assumes the CI role."
  type        = string
}
...
variable "argocd_ingress_enabled" {
  description = "Create the dedicated Argo CD ALB security group. Operator applies k8s/argocd/ingress/{env}.yaml."
  type        = bool
}

variable "shared_alb" {
  description = "true = one ALB for app+ops (dev). false = public app ALB + ops ALB (prod)."
  type        = bool
  default     = false
}
