...
# Purpose: Dev root — wires every module for the petclinic-dev stack.
#
# Flow: vpc -> ecr/rds/eks/secrets/github-oidc -> dns/karpenter/observability
# (Helm add-ons gated by install_* for wave 1 vs wave 2).
#
...
# -----------------------------------------------------------------------------
# 2) VPC - public ALB/NAT, private EKS/RDS, one NAT Gateway (10.0.0.0/16)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 3) EKS - petclinic-dev cluster, t4g.medium nodes in private subnets
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 8) GitHub Actions OIDC - account-scoped CI role (create only from this root)
# -----------------------------------------------------------------------------
